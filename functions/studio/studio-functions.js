...
// 6. Optional: Use method chaining to reduce the lines of code within the function.
    function reverseCharacters(variable){
        if(typeof variable === 'string'){
            ...
            let reveredString = String(variable).split('').reverse().join('');
            return Number(reveredString);
            }
    }

    //myVariableName = 'radar';
...
function completeReversal(inputArray){
    ...
    for(let i =0;i<inputArray.length;i++)
        {
            finalArray.push(reverseCharacters(inputArray[i]));
        }
...
function funPhrase(inputString){
    if(inputString.length <= 3)
    {
        ...
        let first3Chars= inputString.slice(0,3);
        return first3Chars;
    }
}
...
function areaOfRectangle(length, width=null){
    let area;
    if(width === null)
        {
            width = length
